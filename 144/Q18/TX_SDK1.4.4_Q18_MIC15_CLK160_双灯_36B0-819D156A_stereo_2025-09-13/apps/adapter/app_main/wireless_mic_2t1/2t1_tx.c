...
const struct __stream_HighSampleRateSingleMicSystem_parm denoise_parm_1 = {
    .AggressFactor = 1.0f,
    .minSuppress = 0.4f,//0.5f,/*!< 最小压制,建议范围[0.04 - 0.5], 越大降得越少 */
    .init_noise_lvl = 0.0f,
        ...
        if(exit_mic_flag)
        {
            ...
            if (wireless_mic_media) {
                ...
                case 1:
                    ...
                    denoise_flag = 0;
                    power_on_led_flash_del();
                    pwm_led_mode_set(PWM_LED1_BREATHE);
        ...
        if(exit_mic_flag)
        {
            ...
            if (wireless_mic_media)
            {
                if(echo_cmd == WIRELESS_MIC_ECHO_OFF)
                {
                    ...
                    echo_flag = 0;
                    /*if(denoise_flag)
                        green_led_on_flash(1);
                    else
                        green_led_off_flash(1);*/
                    green_led_off_flash(3);
                ...
                if(echo_cmd == WIRELESS_MIC_ECHO_ON)
                {
                    ...
                    echo_flag = 1;
                    /*if(denoise_flag)
                        green_led_on_flash(3);
                    else
                        green_led_off_flash(3);*/
                    green_led_off_flash(2);
