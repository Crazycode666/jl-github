...
void pwm_led_on(u32 gpio_port)
{
    ...
    timer_pwm_init(JL_TIMER3,gpio_port,500,1500);
}
...
void red_led_flash()
{
    gpio_disable_fun_output_port(TCFG_PWMLED_PIN3);
    pwm_led_on(TCFG_PWMLED_PIN3);
    os_time_dly(20);
...
static u16 power_on_led_flash_time = 0;

void power_on_led_flash()
{
    pwm_led_mode_set(PWM_LED1_ON);
    pwm_led_on(TCFG_PWMLED_PIN3);
    os_time_dly(15);
    pwm_led_mode_set(PWM_LED1_OFF);
    pwm_led_off(TCFG_PWMLED_PIN3);
    os_time_dly(15);

}


void power_on_led_flash_add()
{
    if(!power_on_led_flash_time)
        power_on_led_flash_time = sys_timer_add(NULL,power_on_led_flash,500);
}


void power_on_led_flash_del()
{
    //gpio_led_high_block(TCFG_PWMLED_PIN);
    //gpio_led_high_block(TCFG_PWMLED_PIN3);
    if(power_on_led_flash_time)
    {
        sys_timer_del(power_on_led_flash_time);
        power_on_led_flash_time = 0;
    }
