...
int adapter_adc_mic_open(u16 sample_rate, u8 gain, u16 irq_points)
{
    ...
    audio_adc_linein_set_gain(&hdl->mic_ch, adc_pga.gain_line);
    if (!adc_pga.MicBoostFlag) {
        audio_adc_linein_0dB_en(1);
    }
    audio_adc_linein2_open(&hdl->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
    audio_adc_linein2_set_gain(&hdl->mic_ch, adc_pga.gain_line);
    if (!adc_pga.MicBoostFlag) {
    ...
    get_audio_dev_gain();
    audio_adc_linein_open(&hdl->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
    audio_adc_linein_set_gain(&hdl->mic_ch, var_gain.mic_gain);
    if (!var_gain.mic_gain_boost) {
        audio_adc_linein_0dB_en(1);
    }
    audio_adc_linein1_open(&hdl->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
    audio_adc_linein1_set_gain(&hdl->mic_ch, var_gain.mic_gain);
    if (!var_gain.mic_gain_boost) {
...
#if CONFIG_CPU_BR28
    audio_adc_mic_set_gain(&hdl->mic_ch, adc_pga.gain_mic);
    if (!adc_pga.MicBoostFlag) {
        audio_adc_mic_0dB_en(1);
    }
#elif CONFIG_CPU_BR30
    get_audio_dev_gain();
    audio_adc_mic_set_gain(&hdl->mic_ch, var_gain.mic_gain);
    if (!var_gain.mic_gain_boost) {
        audio_mic_0dB_en(1);
    }
#endif
