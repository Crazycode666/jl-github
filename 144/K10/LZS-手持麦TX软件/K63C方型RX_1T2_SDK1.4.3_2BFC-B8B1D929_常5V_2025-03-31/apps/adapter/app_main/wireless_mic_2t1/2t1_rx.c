...
u8 user_dig_vol_step = USER_DIGITAL_VOL_MAX - 3;
...
static void audio_stream_expand_run(struct audio_data_frame *in)
{
    ...
    user_audio_digital_vol_run(in->data, in->data_len, user_dig_vol_table[user_dig_vol_step],user_dig_vol_table[user_dig_vol_step]);
    while (1) {
...
static u8 vol = 10;
static int exit_mic_flag = 1;
static u8 rx_conn_status = 0;
...
extern struct adapter_audio_stream *analog_vol_set;
    ...
    case  KEY_EXIT_MIC:
            if(rx_conn_status)
            {
                if(exit_mic_flag)
                {
                    ...
                    //pro = adapter_process_open(idev, odev, media, event_handle_callback);
                    //audio_sound_dac_set_analog_gain(analog_vol_set->dac1, 0xF, -10);//设置音量
                    //audio_sound_dac_set_digital_gain(analog_vol_set->dac1, 0xF, 0);
                    user_dig_vol_step = 0;
                    Rgb_led_off(TCFG_PWMLED_PIN3);
                    Rgb_led_off(TCFG_PWMLED_PIN2);
                    Rgb_led_on(TCFG_PWMLED_PIN1);
                ...
                {
                    printf("!!!!!!!!!!!!!!!!!222222222222\n");
                    //odev = adapter_odev_open(ADAPTER_ODEV_USB, NULL);
                    //media = adapter_media_open((struct adapter_media_config *)&master_media_config);

                    //audio_sound_dac_set_analog_gain(analog_vol_set->dac1, 0xF, 10);//设置音量
                    //audio_sound_dac_set_digital_gain(analog_vol_set->dac1, 0xF, 16384);
                    user_dig_vol_step = 7;
                    Rgb_led_off(TCFG_PWMLED_PIN1);
                    Rgb_led_off(TCFG_PWMLED_PIN3);
                    Rgb_led_on(TCFG_PWMLED_PIN2);
    ...
    case KEY_STE_VOL_UP:
            if(rx_conn_status)
            {
                if(exit_mic_flag)
                {
                    ...
                    user_dig_vol_step += 3;
                    if(user_dig_vol_step == 7)
                    {
                        Rgb_led_off(TCFG_PWMLED_PIN1);
                        Rgb_led_off(TCFG_PWMLED_PIN3);
                        Rgb_led_on(TCFG_PWMLED_PIN2);
                    }
                    else if(user_dig_vol_step == 10)
                    {
                        Rgb_led_off(TCFG_PWMLED_PIN1);
                        Rgb_led_off(TCFG_PWMLED_PIN2);
                        Rgb_led_on(TCFG_PWMLED_PIN3);
            ...
            break;
    case KEY_STE_VOL_DOWN:
            if(rx_conn_status)
            {
                if(exit_mic_flag)
                {
                   if(user_dig_vol_step > 4)
                    user_dig_vol_step -= 3;
                    if(user_dig_vol_step == 4)
                    {
                        Rgb_led_off(TCFG_PWMLED_PIN3);
        ...
        case DEVICE_EVENT_FROM_ADAPTER:
            ...
            case ADAPTER_EVENT_CONNECT :
                ...
                if (event->u.dev.value == 1) {
                    ...
                    wireless_conn_status |= BIT(0);//00001
                } else {
                    printf("ADAPTER_EVENT_CONNECT_SECOND\n");
                    wireless_conn_status |= BIT(1);//00011
                }
                rx_conn_status = 1;
                led_flash_del();
            ...
            case ADAPTER_EVENT_DISCONN :
                ...
                if (event->u.dev.value == 1) {
                    ...
                    wireless_conn_status &= ~BIT(0);//00010
                } else {
                    printf("ADAPTER_EVENT_DISCONN_SECOND\n");
                    wireless_conn_status &= ~BIT(1);//00000
                }
                if(dev_num == 0)
                {
                    rx_conn_status = 0;
...
void app_main_run(void)
{
    ...
    while (1) {
        //初始化
        idev = adapter_idev_open(ADAPTER_IDEV_BT, (void *)&idev_bt_parm_list);
#if (WIRELESS_MIC_RX_OUTPUT_SEL == WIRELESS_MIC_RX_OUTPUT_USB_MIC)
        odev = adapter_odev_open(ADAPTER_ODEV_USB, NULL);
#else
        odev = adapter_odev_open(ADAPTER_ODEV_DAC, NULL);
#endif
        media = adapter_media_open((struct adapter_media_config *)&master_media_config);
#if WIRELESS_TX_MIC_STEREO_OUTPUT
        //默认输出选择1:真立体 0:假立体
