        ...
        if(connect_state_flag)
        {
            if(exit_mic_flag)
            {
                ...
                if(user_dig_vol_step == 10)
                {
                    ...
                    pwm_led_mode_set(PWM_LED1_OFF);
                    echo_led_on(TCFG_PWMLED_PIN0);
                    syscfg_write(CFG_USER_MIC_VOL,&user_dig_vol_step,10);
                }
                else if(user_dig_vol_step == 7)
                {
                    printf("默认音量\n");
                    ...
                    pwm_led_mode_set(PWM_LED1_OFF);
                    echo_led_on(TCFG_PWMLED_PIN3);
                    syscfg_write(CFG_USER_MIC_VOL,&user_dig_vol_step,7);
                }
                else if(user_dig_vol_step == 4)
                {
                    printf("最小音量\n");
                    ...
                    pwm_led_mode_set(PWM_LED1_ON);
                    //echo_led_on(TCFG_PWMLED_PIN3);
                    syscfg_write(CFG_USER_MIC_VOL,&user_dig_vol_step,4);
    ...
    case SYS_DEVICE_EVENT:
        ...
        case DEVICE_EVENT_FROM_ADAPTER:
            ...
            case ADAPTER_EVENT_CONNECT :
                ...
                //user_dig_vol_step = mic_vol_default;
                syscfg_read(CFG_USER_MIC_VOL,&user_dig_vol_step,user_dig_vol_step);
