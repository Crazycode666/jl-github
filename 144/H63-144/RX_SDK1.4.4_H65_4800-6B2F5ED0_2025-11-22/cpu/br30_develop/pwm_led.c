...
void led_on(u32 led_gpio)
{
    gpio_set_hd0(led_gpio, 1);
    gpio_set_hd(led_gpio, 1);
    gpio_set_pull_up(led_gpio,1);
    gpio_set_direction(led_gpio, 0);
    //gpio_set_output_value(led_gpio, 1);
    gpio_direction_output(led_gpio, 1);

}


void user_pwm_led_on(u32 gpio_port,JL_TIMER_TypeDef *JL_TIMERx)
{
    timer_pwm_init(JL_TIMERx,gpio_port,500,5000);
    //timer_pwm_init(JL_TIMER2,TCFG_PWMLED_PIN1,200,0);
}
...
void led_flash()
{
    ...
    user_pwm_led_on(TCFG_PWMLED_PIN0,JL_TIMER3);
    user_pwm_led_on(TCFG_PWMLED_PIN1,JL_TIMER2);
    os_time_dly(20);
...
void led_flash_add()
{
    if(!led_time)
    {
        led_time = sys_timer_add(NULL,led_flash,600);
    }
}
...
void blue_led_flash()
{
    led_off(TCFG_PWMLED_PIN0);
    os_time_dly(20);
    led_on(TCFG_PWMLED_PIN0);
    os_time_dly(20);
}

static u32 blue_flash_time = 0;
void blue_led_flash_add()
{
    if(!blue_flash_time)
    {
        blue_flash_time = sys_timer_add(NULL, blue_led_flash, 600);
    }
}

void blue_led_flash_del()
{

    if(blue_flash_time)
    {
        sys_timer_del(blue_flash_time);
        blue_flash_time = 0;
    }
}
///////////////////////////////////////////////////////////////////////////
