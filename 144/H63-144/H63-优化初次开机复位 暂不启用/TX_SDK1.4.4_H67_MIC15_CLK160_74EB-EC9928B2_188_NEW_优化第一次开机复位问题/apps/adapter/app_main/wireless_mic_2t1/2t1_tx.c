...
const struct __stream_HighSampleRateSingleMicSystem_parm denoise_parm_1 = {
    .AggressFactor = 1.0f,
    .minSuppress = 0.35f,//0.5f,/*!< 最小压制,建议范围[0.04 - 0.5], 越大降得越少 */
    .init_noise_lvl = 0.0f,
...
const struct __stream_HighSampleRateSingleMicSystem_parm denoise_parm_3 = {
    .AggressFactor = 1.0f,
    .minSuppress = 0.04f,/*!< 最小压制,建议范围[0.04 - 0.5], 越大降得越少 */
    .init_noise_lvl = 0.0f,
    .frame_len = WIRELESS_CODING_FRAME_LEN,
};
#endif
...
u16 led_control_flag = 1;

static u16 led7_display_cnt = 0;
...
static int adapter_key_event_handler(struct sys_event *event)
{
    ...
    break;
    case  KEY_LED_DISPLAY:
        //if(key_poweroff_cnt >= LED7_DISPLAY_TIME)
            {
                if(!led_control_flag)
                {
                    led_control_flag = 1;
