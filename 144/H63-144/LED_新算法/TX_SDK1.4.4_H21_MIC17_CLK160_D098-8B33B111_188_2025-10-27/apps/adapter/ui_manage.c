...
void green_led_on_flash(u8 flag)
{
...
void green_led_off_flash(u8 flag)
{
    for(int i = 0;i < flag;i++)
    {
...
int poweron_vbat_value = 0;
extern int power_reset_src;
...
void ui_manage_scan(void *priv)
{
    ...
    case STATUS_POWERON:
       ...
       //echo_led_on(IO_PORTC_03);
        ...
        //syscfg_read(USER_CFG_FIRST_POWERON,&is_first_poweron,1);
        //syscfg_read(USER_CFG_CHARGE_VALUE,&led7_init_num,1);
        printf("----is_first_poweron = %d------\n",is_first_poweron[0]);
        printf("$$$$$$$$$$$$$$$$$  led7_init_num[0] = %d &&&&&&&&&&&&&&\n",led7_init_num[0]);
        #endif // 0

        poweron_vbat_value = get_vbat_percent();
        /*printf("$$$$$$$$$$$$$$$$$  poweron_vbat_value = %d &&&&&&&&&&&&&&\n",poweron_vbat_value);
        //led7_init_num[0] = 0;
        printf("(led7_init_num[0] - poweron_vbat_value) = %d\n",(led7_init_num[0] - poweron_vbat_value));
        if(is_first_poweron[0] || ((led7_init_num[0] - poweron_vbat_value) >= 5))
            led7_init_num[0] = poweron_vbat_value;
*/
        printf("$$$$$$$$$$$$$$$$$  led7_init_num = %d &&&&&&&&&&&&&&\n",led7_init_num[0]);
        //pwm_led_on(TCFG_PWMLED_PIN1);
        led7_init_num[0] = get_vbat_percent();
        //charge_vbat_led_states();
        hundreds = 3;
            tens = 8;
            unit = 8;
        led_water_flash_add();
        ...
        vbat_display_delay_add();
        //bat_level_display_add();
        led_light_time_display_add();
        //low_bat_poweroff_dete_add();
