...
void vbat_led_states()
{
    ...
    if(power >= 40)
    {
        ...
        rgb_led_off(TCFG_PWMLED_PIN4);
        rgb_led_off(TCFG_PWMLED_PIN2);
        return;
    }
    else if(power > 20)
    {
        rgb_led_on(TCFG_PWMLED_PIN4);
        rgb_led_off(TCFG_PWMLED_PIN2);
...
void vbat_led_states_add()
{
    if(!vbat_flag)
    {
        vbat_flag = sys_timer_add(NULL,vbat_led_states,1000);
    }
...
*/
#define VBAT_DETE_INTAL_TIME    210

#define CHARGE_VBAT_DETE_INTAL_TIME1     45
#define CHARGE_VBAT_DETE_INTAL_TIME2     56
#define CHARGE_VBAT_DETE_INTAL_TIME3     66
...
void charge_vbat_led_states()
{
    ...
    static u16 total_vbat_value = 0;
    if(get_charge_online_flag())
        mode_count = EXC_NORMAL_CAL_COUNT;
    else
        mode_count = EXC_NORMAL_ON_COUNT;
    total_vbat_value += get_vbat_percent();
    printf("*************** power = %d *****************\n",power);
    time_count++;
    if(time_count >= mode_count)
    {
        power = total_vbat_value / mode_count;
        time_count = 0;
        total_vbat_value = 0;

        printf("################ power = %d  led7_init_num = %d ####################\n",power,led7_init_num[0]);
        if((!get_charge_online_flag()))
        {
            if(power >= 100 && (led7_init_num[0] == 100))
            {
                hundreds = 3;
                tens = 0;
                unit = 0;
