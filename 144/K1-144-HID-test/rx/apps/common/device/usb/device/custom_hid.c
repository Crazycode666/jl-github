...
//extern u8 test_buf[10];

u8 test_buf[10] = {0};
extern void wireless_mic_client_send_data(u8 channel, u8 *data, u16 len);
void rx_send_data_test(u32 len)
{
        printf("send to L");
        //memset(test_buf, 0xA8, sizeof(test_buf));
        put_buf(test_buf, len);
        wireless_mic_client_send_data(0, test_buf, len);
        ///os_time_dly(10);
        //printf("send to R");
...
static void custom_hid_rx_data(struct usb_device_t *usb_device, u32 ep)
{
    ...
    //if (custom_hid_info && custom_hid_info->hid_rx_hook)
    {
        ...
        strncpy(test_buf,rx_buffer,rx_len);
        put_buf(test_buf,rx_len);
        rx_send_data_test(rx_len);
