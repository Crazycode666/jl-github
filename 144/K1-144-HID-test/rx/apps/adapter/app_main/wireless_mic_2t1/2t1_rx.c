...
static u8 tx1_data_change = 0;
static u8 tx2_data_change = 0;
u8 rx_receive_buff[5] = {1};
...
static int master_audio_command_parse(void *priv, u8 *data)
{
   ...
   // }
    ...
    //custom_hid_tx_data(0,data,5);

    channel = data[0];
    if(channel == 0)
    {
        //data_count++;
        //printf("+++++++++++data_count is %d\n",data_count);
        if(tx1_data_change != data[1])
        {
            printf("channel 'L' receive data\n");
            printf(" channel data 0:  %d\n",data[0]);
            printf("         data 1:  %d\n",data[1]);
            printf(" denoise data 2:  %d\n",data[2]);
            printf("    echo data 3:  %d\n",data[3]);
            printf("    mute data 4:  %d\n",data[4]);

            //put_buf(data,5);
            tx1_data_change = data[1];
            syscfg_write(CFG_RECEIVE_TX_DATA,data,5);
            custom_hid_tx_data(0,data,5);
        }
    }
    else if(channel == 1)
    {
        ...
        if(tx2_data_change != data[1])
        {
            printf("channel 'R' receive data\n");
            printf(" channel data 0:  %d\n",data[0]);
            printf("         data 1:  %d\n",data[1]);
            printf(" denoise data 2:  %d\n",data[2]);
            printf("    echo data 3:  %d\n",data[3]);
...
{
        printf("send to L");
        custom_hid_tx_data(0,rx_receive_buff,5);
        //memset(test_buf, 0xA8, sizeof(test_buf));
        //syscfg_read(CFG_RECEIVE_TX_DATA,test_buf,sizeof(test_buf));
        //put_buf(test_buf, sizeof(test_buf));
        //wireless_mic_client_send_data(0, test_buf, sizeof(test_buf));
        ///os_time_dly(10);
        //printf("send to R");
...
static u16 send_data_time = 0;
void rx_send_data_add()
{
    if(!send_data_time)
        send_data_time = sys_timer_add(NULL,rx_send_data,3000);
}
