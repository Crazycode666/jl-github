...
#define SLAVE_ENC_COMMAND_LEN		5

static struct adapter_media *wireless_mic_media = NULL;
static u8 command = WIRELESS_MIC_DENOISE_LEVEL_1;
...
static u8 tx1_send_data = 0;
static u8 tx2_send_data = 0;
static void slave_audio_encode_callback(u8 *data, u16 len)
{
    ...
    data[0] = (u8)tx_channel;
    if(data[0] == 0)
    {
        data[1] = (u8)tx_cmd;
        if(data[1] != tx1_send_data)
        {
            data[2] = (u8)command;
            os_time_dly(3);
            data[3] = (u8)echo_cmd;
            os_time_dly(3);
            data[4] = (u8)exit_mic_flag;
            os_time_dly(3);
            printf("channel 'L' send data\n");
            put_buf(data,5);
            tx1_send_data = data[1];
        }
        ...
        if(data[1] != tx2_send_data)
        {
            ...
            data[4] = (u8)exit_mic_flag;
            os_time_dly(3);
            printf("channel 'R' send data\n");
            put_buf(data,5);
            tx2_send_data = data[1];
        }
...
void wireless_mic_server_recieve_data(void *priv, void *data, u16 len)
{
    ...
    {
        case 0:
            printf("KEY_WIRELESS_MIC_DENOISE_SET\n");
            app_task_put_key_msg(KEY_WIRELESS_MIC_DENOISE_SET,0);
            break;
        case 1:
            printf("KEY_WIRELESS_MIC_ECHO_SET\n");
            app_task_put_key_msg(KEY_WIRELESS_MIC_ECHO_SET,0);
            break;
        case 2:
            printf("KEY_EXIT_MIC\n");
            app_task_put_key_msg(KEY_EXIT_MIC,0);
            break;
...
void channel_set_data_dete()
{
    ...
    for (int i = 0; i < 5; i++) {
        os_time_dly(2);
        if (gpio_read(IO_PORTB_02)) {
            count++;
        }
    }
    tx_channel = (count >= 3) ? 0 : 1;
    printf("###########the channel is %d  #######\n",tx_channel);
}

static u16 channel_diff_time = 0;


void channel_set_data_dete_add()
{
    if(!channel_diff_time)
    {
        channel_diff_time = sys_timer_add(NULL,channel_set_data_dete,1000);
    }
}
static int event_handle_callback(struct sys_event *event)
{
...
void app_main_run(void)
{
    ...
    while (1) {
        ...
        ASSERT(odev);
        ASSERT(media);
        //channel_set_data_dete_add();
        channel_set_data_dete();
