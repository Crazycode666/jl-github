...
void charge_ui_set()
{
    led_water_flash_del();
        ...
        os_time_dly(120);
        led_water_flash_del();
        charge_init_dete_value = get_vbat_percent();
        syscfg_read(USER_CFG_FIRST_POWERON,&is_first_poweron,1);
        syscfg_read(USER_CFG_CHARGE_VALUE,&led7_init_num,1);
        if(is_first_poweron[0] == 1 || charge_init_dete_value <= led7_init_num[0])
        {
            led7_init_num[0] = charge_init_dete_value;
        ...
        led_water_flash_add();
        //led7_charge_flash1_add();
        //charge_vbat_led_states_add();
       // bat_level_display_add();
       charge_vbat_display_delay_add();
}
...
static int charge_off_time = 0;
#define CHARGE_DEALY_TIME 20

void charge_off_timer_add()
{
    if(!charge_off_time)
    {
        charge_off_time = sys_timer_add(NULL,charge_off_timer,1000*CHARGE_DEALY_TIME);
    }
}


void charge_off_timer_del()
{
    if(charge_off_time)
    {
        sys_timer_del(charge_off_time);
        charge_off_time = 0;
    }
}
