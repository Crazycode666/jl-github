...
void pwm_led_on(u32 gpio_port)
{

    timer_pwm_init(JL_TIMER3,gpio_port,500,3000);
}
...
void pwm_led_off(u32 gpio_port)
{

    gpio_disable_fun_output_port(gpio_port);
    gpio_led_high_block(gpio_port);
}



static u16 red_led_time = 0;

void red_led_flash()
{
    gpio_disable_fun_output_port(TCFG_PWMLED_PIN2);
    pwm_led_on(TCFG_PWMLED_PIN2);
    os_time_dly(20);
    gpio_led_high_block(TCFG_PWMLED_PIN2);
}

void red_led_flash_add()
{
    if(!red_led_time)
    {
        red_led_time = sys_timer_add(NULL,red_led_flash,10000);
    }
}

void red_led_flash_del()
{
    if(red_led_time)
    {
        sys_timer_del(red_led_time);
        red_led_time = 0;
    }
}
...
{
    #if 1
    gpio_led_high_block(IO_PORTC_02);
    gpio_led_high_block(IO_PORTB_08);
    gpio_led_high_block(IO_PORTB_06);
    gpio_led_high_block(IO_PORTB_03);
    gpio_led_high_block(IO_PORTB_02);
    #else
    echo_led_off(IO_PORTC_02);
    echo_led_off(IO_PORTB_08);
    echo_led_off(IO_PORTB_06);
    echo_led_off(IO_PORTB_03);
    echo_led_off(IO_PORTB_02);
    #endif
}
...
{
    #if 1
    echo_led_off(IO_PORTC_02);				//拉低Pin1
    if(nums[unit] & BIT(5))	//B3
    	echo_led_on(IO_PORTB_08);
    if(nums[unit] & BIT(3))	//D3
		echo_led_on(IO_PORTB_06);
	if(nums[unit] & BIT(1))	//F3
		echo_led_on(IO_PORTB_03);
	if(nums[unit] & BIT(0))	//G3
		echo_led_on(IO_PORTB_02);
    #else
    pwm_led_off(IO_PORTC_02);				//拉低Pin1
    if(nums[unit] & BIT(5))	//B3
    	pwm_led_on(IO_PORTB_08);
    if(nums[unit] & BIT(3))	//D3
		pwm_led_on(IO_PORTB_06);
	if(nums[unit] & BIT(1))	//F3
		pwm_led_on(IO_PORTB_03);
	if(nums[unit] & BIT(0))	//G3
		pwm_led_on(IO_PORTB_02);
    #endif
}
...
void Display_Scan2(void)
{
	...
	if(nums[tens] & BIT(2))	//E2
		echo_led_on(IO_PORTB_02);
    #else
    pwm_led_off(IO_PORTB_08);
    if(nums[unit] & BIT(6))	//A3
    	pwm_led_on(IO_PORTC_02);
	if(nums[tens] & BIT(5))	//B2
		pwm_led_on(IO_PORTB_06);
	if(nums[tens] & BIT(3))	//D2
		pwm_led_on(IO_PORTB_03);
...
void led7_charge_flash_add()
{
    if(!led7_charge_flash_time)
        led7_charge_flash_time = sys_timer_add(NULL,led7_charge_flash,1200);
}
...
static u16 led7_charge_flash_time1 = 0;
void led7_charge_flash1_add()
{
    if(!led7_charge_flash_time1)
        led7_charge_flash_time1 = sys_timer_add(NULL,led7_charge_flash,600);
}

void led7_charge_flash1_del()
{
    if(led7_charge_flash_time1)
    {
        sys_timer_del(led7_charge_flash_time1);
        led7_charge_flash_time1 = 0;
    }
}
...
static u16 led_light_time = 0;

void led_light_time_display()
{
    if(led_control_flag)
    {
...
void led_light_time_display_add()
{
    if(!led_light_time)
        led_light_time = sys_timer_add(NULL,led_light_time_display,10000);
}

void led_light_time_display_del()
{
    if(led_light_time)
    {
        sys_timer_del(led_light_time);
        led_light_time = 0;
    }
}
...
void vbat_display_delay()
{
    user_vbat_init();
    user_vbat_display_add();
}

void vbat_display_delay_add()
{
    sys_timeout_add(NULL,vbat_display_delay,800);
}


void charge_vbat_display_delay()
{
    user_vbat_init();
...
void charge_vbat_display_delay_add()
{
    sys_timeout_add(NULL,charge_vbat_display_delay,800);
}
