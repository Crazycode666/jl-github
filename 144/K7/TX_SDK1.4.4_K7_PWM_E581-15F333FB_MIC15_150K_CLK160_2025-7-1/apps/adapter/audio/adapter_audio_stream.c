...
extern void pause_noisegate(NOISEGATE_API_STRUCT *noisegate_hdl, u8 run_mark);
extern void close_noisegate(NOISEGATE_API_STRUCT *noisegate_hdl);
extern void audio_noisegate_bypass(u32 ns_name, u8 bypass);
void adapter_audio_stream_noisegain_switch(struct adapter_audio_stream *audio,u8 onoff)
{

#if WIRELESS_DENOISE_ENABLE
    if(audio && audio->noisegate)
    {
        onoff = !onoff;
        audio_noisegate_bypass(audio->noisegate,onoff);
       //close_noisegate(audio->noisegate);
    }
#endif // WIRELESS_DENOISE_ENABLE
}
