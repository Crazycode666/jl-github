...
const struct __stream_HighSampleRateSingleMicSystem_parm denoise_parm_1 = {
    .AggressFactor = 1.0f,
    .minSuppress = 0.6f,//0.5f,/*!< 最小压制,建议范围[0.04 - 0.5], 越大降得越少 */
    .init_noise_lvl = 0.0f,
...
static u8 denoise_flag = 1;
static u8 echo_flag = 0;
u16 user_pwm_test_value = 0;
...
static int adapter_key_event_handler(struct sys_event *event)
{
        ...
        if(exit_mic_flag)
        {
            ...
            if (wireless_mic_media) {
                ...
                case 1:
                    ...
                    adapter_decoder_ioctrl(wireless_mic_media->downdecode, ADAPTER_DEC_IOCTRL_CMD_NOSIGATE_SWITCH, 1, (int *)1);
                ...
                case 2:
                    //深度降噪
                    adapter_decoder_ioctrl(wireless_mic_media->downdecode, ADAPTER_DEC_IOCTRL_CMD_DENOISE_SWITCH, 1, (int *)1);
                    adapter_decoder_ioctrl(wireless_mic_media->downdecode, ADAPTER_DEC_IOCTRL_CMD_NOSIGATE_SWITCH, 1, (int *)0);
            ...
            if (wireless_mic_media)
            {
                ...
                if(echo_cmd == WIRELESS_MIC_ECHO_ON)
                {
                    ...
                    /*if(denoise_flag)
                        ...
                        green_led_flash(1);*/
                        user_pwm_led_on(TCFG_PWMLED_PIN2);
                }
            }
        }
        break;
    ...
    break;
    case KEY_USER_PWM_CTL:
        /*void set_timer_pwm_duty(JL_TIMER_TypeDef * JL_TIMERx,u32 duty);
        user_pwm_test_value+= 2000;
        if(user_pwm_test_value >10000)
            user_pwm_test_value = 0;
        set_timer_pwm_duty(JL_TIMER4,user_pwm_test_value);
        set_timer_pwm_duty(JL_TIMER2,10000 - user_pwm_test_value);*/
        static u8 pwm_flag = 0;
        if(pwm_flag)
            gpio_set_fun_output_port(TCFG_PWMLED_PIN0, FO_TMR4_PWM, 0, 0);
        else
            gpio_set_fun_output_port(TCFG_PWMLED_PIN0, FO_TMR4_PWM, 0, 1);
        pwm_flag = !pwm_flag;
        break;
    case  KEY_POWEROFF:
        printf("KEY POWEROFF\n");
