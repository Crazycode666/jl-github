...
{
    timer_pwm_init(JL_TIMER4,gpio_port,200,4000);
    //timer_pwm_init(JL_TIMER2,TCFG_PWMLED_PIN1,200,0);
}
...
void fast_led_flash_add()
{
    if(!fast_led_time)
    {
        fast_led_time = sys_timer_add(NULL,fast_led_flash,400);
    }
}

void fast_led_flash_del()
{
    if(fast_led_time)
    {
        sys_timer_del(fast_led_time);
        fast_led_time = 0;
    }
}
