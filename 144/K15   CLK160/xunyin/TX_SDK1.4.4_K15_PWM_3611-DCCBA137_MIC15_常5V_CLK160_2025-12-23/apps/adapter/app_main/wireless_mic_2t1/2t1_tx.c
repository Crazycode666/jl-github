...
static bool is_default_vol = true;
static int event_handle_callback(struct sys_event *event);

extern u8 flag_key_sw_recorder;
extern void fast_led_flash_add();
extern void fast_led_flash_del();
...
static int adapter_key_event_handler(struct sys_event *event)
{
    ...
    case KEY_CHANGE_VOL:
        if(connect_state_flag)
        {
            if(exit_mic_flag)
            {
                extern void adapter_adc_gain_set(u8 mic_gain, u8 linein_gain);

                is_default_vol = !is_default_vol;
                if(!is_default_vol)
                {
                    adapter_adc_gain_set(17,0);
                    gpio_disable_fun_output_port(TCFG_PWMLED_PIN2);
                    rgb_led_off(TCFG_PWMLED_PIN2);
                    rgb_led_on(TCFG_PWMLED_PIN1);
                }
