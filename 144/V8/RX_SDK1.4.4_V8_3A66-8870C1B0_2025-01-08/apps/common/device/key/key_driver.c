...
#ifdef MY_POWER_PIN_IO

#define MY_POWER_PIN_READ_READ()  gpio_read(MY_POWER_PIN_IO)
static u32 hold_up_cnt = 0;
static void check_power_pb1_on(void)
{
    gpio_set_pull_down(MY_POWER_PIN_IO, 0);//看需求是否需要开内部下拉
    gpio_set_pull_up(MY_POWER_PIN_IO, 0);//看需求是否需要开内部下拉
    gpio_set_die(MY_POWER_PIN_IO, 1);
    gpio_set_direction(MY_POWER_PIN_IO, 1);
    delay(40);//设置方向寄存器后,不能立马读电压
#if POWER_PIN_IO_HIGH
    if (!MY_POWER_PIN_READ_READ()) {
#else
    if (MY_POWER_PIN_READ_READ()) {
#endif
        putchar('+');
