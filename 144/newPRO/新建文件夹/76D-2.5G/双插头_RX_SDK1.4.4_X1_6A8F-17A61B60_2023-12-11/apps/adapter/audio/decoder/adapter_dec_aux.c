#include "adapter_dec_aux.h"
#include "adapter_decoder.h"
#include "adapter_audio_stream.h"
#include "adapter_adc.h"
#include "media/pcm_decoder.h"
#include "clock_cfg.h"
#include "audio_config.h"
#include "asm/dac.h"
#include "adapter_wireless_command.h"

#define ADC_PCM_FOR_LC3_EN				1

#if (TCFG_AUDIO_ADC_ENABLE)
struct __adapter_dec_aux {
    struct adapter_media_parm 		*media_parm;// 动态参数
    struct adapter_decoder_fmt  	*fmt;		// 解码参数配置
    void						 	*stream;	// 音频流
    struct adapter_audio_stream		*adapter_stream;// 音频流
    struct pcm_decoder 				pcm_dec;	// pcm解码句柄
    struct audio_res_wait 			wait;		// 资源等待句柄
    volatile u8 					status;		// 正在解码
    u16 							sample_rate;// 采样率
    struct audio_mixer_ch           mix_ch;    // 叠加句柄
    struct audio_mixer              *mixer;
    u8                              mixer_en;
    struct channel_switch *channel_zoom;		/*!< 降噪节点控制句柄 */
};
struct __adapter_dec_aux *dec_aux = NULL;
#define __this dec_aux


const struct __stream_HighSampleRateSingleMicSystem_parm denoise_parm = {
    .AggressFactor = 1.0f,
    .minSuppress = 0.5f,//0.5f,/*!< 最小压制,建议范围[0.04 - 0.5], 越大降得越少 */
    .init_noise_lvl = 0.0f,
};

u16 adapter_dec_aux_get_samplerate(void *priv)
{
    u16 sample_rate = 0;
    if (__this) {
        sample_rate = __this->sample_rate;
    }
    return sample_rate;
}

static void adapter_dec_aux_resume(void *priv)
{
    if (__this && __this->status) {
        //putchar('1');
        audio_decoder_resume(&__this->pcm_dec.decoder);
    }
}

static void adapter_dec_aux_event_handler(struct audio_decoder *decoder, int argc, int *argv)
{
    return ;
}

static int adapter_dec_aux_stop(void)
{
    if (!__this || !__this->status) {
        return 0;
    }

    __this->status = 0;
    adapter_adc_mic_close();
    pcm_decoder_close(&__this->pcm_dec);
    //app_audio_state_exit(APP_AUDIO_STATE_MUSIC);

    //关闭数据流

    if (__this->mixer) {
        if (__this->stream) {
            audio_mixer_ch_close(&__this->mix_ch);
            audio_stream_close((struct audio_stream *)__this->stream);
            __this->stream = NULL;
        }
    } else {
        adapter_audio_stream_close((struct adapter_audio_stream **)&__this->stream);
    }
    //clock_set_cur();
    return 0;
}

static int adapter_dec_aux_start(void)
{
    int err;
    if (!__this) {
        return -EINVAL;
    }

#if(ADC_PCM_FOR_LC3_EN)
    adapter_adc_mic_open(WIRELESS_CODING_SAMPLERATE, WIRELESS_MIC_ADC_GAIN, WIRELESS_MIC_ADC_POINT_UNIT);
    //adapter_adc_mic_open(44100, 10, 120);
#else
    adapter_adc_mic_open(WIRELESS_CODING_SAMPLERATE, WIRELESS_MIC_ADC_GAIN, WIRELESS_MIC_ADC_POINT_UNIT);
#endif

    adapter_adc_mic_data_output_resume_register(adapter_dec_aux_resume, NULL);

    // 打开pcm解码器
    err = pcm_decoder_open(&__this->pcm_dec, adapter_decoder_get_task_handle());
    if (err) {
        adapter_adc_mic_close();
        pcm_decoder_close(&__this->pcm_dec);
        return -1;
    }

    pcm_decoder_set_event_handler(&__this->pcm_dec, adapter_dec_aux_event_handler, 0);
    pcm_decoder_set_read_data(&__this->pcm_dec, adapter_adc_mic_data_read, NULL);

    __this->mixer_en = 0;

    if (__this->mixer) {
        audio_mixer_ch_open(&__this->mix_ch, __this->mixer);
        //audio_mixer_ch_set_no_wait(&dec->mix_ch, 1, 18); // 超时自动丢数

        struct audio_stream_entry *entries[8] = {NULL};
        u8 entry_cnt = 0;
        entries[entry_cnt++] = &__this->pcm_dec.decoder.entry;

        /*__this->channel_zoom = channel_switch_open(AUDIO_CH_DIFF, 0);
        if(__this->channel_zoom){
            entries[entry_cnt++] = &__this->channel_zoom->entry;
        }*/

        entries[entry_cnt++] = &__this->mix_ch.entry;
        __this->stream = (void *)audio_stream_open(&__this->pcm_dec.decoder, adapter_dec_aux_resume);
        audio_stream_add_list(__this->stream, entries, entry_cnt);

        __this->mixer_en = 1;

        printf("%s output  mixer\n", __FUNCTION__);
    } else {
        //数据流初始化
        __this->stream = (void *)adapter_audio_stream_open(
                             __this->fmt->list,
                             __this->fmt->list_num,
                             &__this->pcm_dec.decoder,
                             adapter_dec_aux_resume,
                             __this->media_parm
                         );
        if (__this->stream == NULL) {
            adapter_adc_mic_close();
            pcm_decoder_close(&__this->pcm_dec);
            return -1;
        }
    }

    // 开始解码
    __this->status = 1;
    err = audio_decoder_start(&__this->pcm_dec.decoder);
    if (err) {
        __this->status = 0;
        adapter_adc_mic_close();
        pcm_decoder_close(&__this->pcm_dec);
        return -1;
    }
    //启动mic
    adapter_adc_mic_start();

    if (audio_dac_init_status()) {
        /*有使能dac 这里建议设置下音量*/
        //audio_output_set_start_volume(APP_AUDIO_STATE_MUSIC);
        app_audio_volume_init();
        app_audio_state_switch(APP_AUDIO_STATE_MUSIC, 15);
        app_audio_set_volume(APP_AUDIO_STATE_MUSIC, 15, 0);
    }

    //clock_set_cur();
    return 0;
}


static int adapter_dec_aux_wait_res_handler(struct audio_res_wait *wait, int event)
{
    int err = 0;

    if (event == AUDIO_RES_GET) {
        // 启动解码
        err = adapter_dec_aux_start();
    } else if (event == AUDIO_RES_PUT) {
        // 被打断
        adapter_dec_aux_stop();
    }

    return err;
}

struct __adapter_dec_aux *adapter_dec_aux_open(
    struct adapter_decoder_fmt *fmt,
    struct adapter_media_parm *media_parm,
    struct audio_mixer *mixer,
    struct adapter_audio_stream	*adapter_stream)
{
    if (__this) {
        printf("adapter_dec_mic_open aready, fail!!\n");
        return 0;
    }
    ASSERT(fmt, "%s, %d, fmt err!!\n", __FUNCTION__, __LINE__);
    struct __adapter_dec_aux*hdl = zalloc(sizeof(struct __adapter_dec_aux));
    if (hdl == NULL) {
        return -1;
    }

    g_printf("adapter_dec_mic_open\n");
    hdl->fmt = fmt;
    hdl->media_parm = media_parm;
    hdl->mixer = mixer;
    hdl->adapter_stream = adapter_stream;

    hdl->pcm_dec.sample_rate = WIRELESS_CODING_SAMPLERATE;//44100L;
    hdl->pcm_dec.ch_num = 1;
    hdl->pcm_dec.output_ch_num = 1;
    hdl->pcm_dec.output_ch_type = AUDIO_CH_DIFF;

    hdl->sample_rate = hdl->pcm_dec.sample_rate;

    hdl->wait.protect = 1;

    hdl->wait.priority = 3;
    hdl->wait.preemption = 0;
    hdl->wait.snatch_same_prio = 1;
    hdl->wait.handler = adapter_dec_aux_wait_res_handler;

    local_irq_disable();
    __this = hdl;
    local_irq_enable();

    audio_decoder_task_add_wait(adapter_decoder_get_task_handle(), &hdl->wait);

    g_printf("adapter_dec_mic_open ok!!!!!!!\n");
    //clock_add(DEC_PCM_CLK);

    return hdl;
}

static void adapter_dec_mic_set_vol(u32 channel, u8 vol)
{
    if (__this && __this->stream) {
        adapter_audio_stream_set_vol(__this->stream, channel, vol);
    }
}

// 解码释放
void adapter_aux_dec_close(struct __adapter_dec_aux **hdl)
{
    if (hdl && (*hdl)) {
        struct __adapter_dec_aux *dec = *hdl;
        adapter_dec_aux_stop();
        audio_decoder_task_del_wait(adapter_decoder_get_task_handle(), &__this->wait);
        /* adapter_wireless_rx_close(&dec->rx); */

        // 释放空间
        local_irq_disable();
        free(__this);
        __this = NULL;
        local_irq_enable();
    }
}

#endif//TCFG_AUDIO_ADC_ENABLE

