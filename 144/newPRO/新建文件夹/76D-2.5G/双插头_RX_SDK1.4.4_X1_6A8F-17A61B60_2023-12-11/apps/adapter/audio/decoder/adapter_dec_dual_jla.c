...
#if USER_SET_RX_MIC_EN
    struct __adapter_wireless_dec *aux_hdl;
#endif
};
...
#if USER_SET_RX_MIC_EN
    media_parm->ch_index = 2;
    hdl->aux_hdl = adapter_dec_aux_open(fmt, media_parm, &hdl->mixer, hdl->stream);
    ASSERT(hdl->aux_hdl);
#endif
...
#if USER_SET_RX_MIC_EN
    adapter_aux_dec_close(&__this->aux_hdl);
#endif
