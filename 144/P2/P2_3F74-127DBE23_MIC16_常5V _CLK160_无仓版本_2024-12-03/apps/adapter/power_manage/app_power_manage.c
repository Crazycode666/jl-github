...
void power_display()
{
    u8 power_value = get_vbat_percent();
    ...
    else if(power_value >= 40)
    {
        echo_led_on(TCFG_PWMLED_PIN0);
        echo_led_on(TCFG_PWMLED_PIN1);
        echo_led_on(TCFG_PWMLED_PIN2);
    }
    else if(power_value >= 20)
    {
        echo_led_on(TCFG_PWMLED_PIN0);
        echo_led_on(TCFG_PWMLED_PIN1);
    }
    else if(power_value >= 5){
        echo_led_on(TCFG_PWMLED_PIN0);
    }
}

/*static u8 power_display_time = 0;
void power_display_add()
{
    if(!power_display_time)
    {
        power_display_time = sys_timer_add(NULL,power_display,200);
    }
}

void power_display_del()
{
    //if(power_display_time)
    {
        sys_timer_del(power_display_time);
        power_display_time = 0;
    }
}
*/

void poweron_led_display()
{
    //power_display_del();
    led_flash_add();
}

static u8 dis_time = 0;

void poweron_led_display_add()
{
    if(!dis_time)
    {
        dis_time = sys_timeout_add(NULL,poweron_led_display,1200);
    }
}

void poweron_led_display_del()
{
    if(dis_time)
    {
        sys_timeout_del(dis_time);
        dis_time = 0;
    }
}
void power_event_to_user(u8 event)
