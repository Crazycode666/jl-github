...
u8 user_dig_vol_step = USER_DIGITAL_VOL_MAX - 2;
...
static int adapter_key_event_handler(struct sys_event *event)
{
    ...
    case KEY_USER_SET_DIG_VOL_UP:
        if(exit_mic_flag)
        {
            user_dig_vol_step += 2;
            if(user_dig_vol_step > USER_DIGITAL_VOL_MAX)
            {
                user_dig_vol_step = 2;
            ...
            if(user_dig_vol_step == 2)
            {
                ...
                gpio_led_high_block(TCFG_PWMLED_PIN3);
                pwm_led_mode_set(PWM_LED1_OFF);
            }
            else if(user_dig_vol_step == 4)
            {
                echo_led_on(TCFG_PWMLED_PIN1);
            }
            else if(user_dig_vol_step == 6)
            {
                echo_led_on(TCFG_PWMLED_PIN2);
            }
            else if(user_dig_vol_step == 8)
            {
            ...
            user_dig_vol_flag = user_dig_vol_step;
        }

        break;
    case KEY_USER_SET_DIG_VOL_DOWN:
        user_dig_vol_step -= 3;
        if(user_dig_vol_step <= 0)
        {
            break;
        }
        break;
    case  KEY_EXIT_MIC:
        if(rx_conn_status)
        {
            ...
            else
            {
                ...
                user_dig_vol_step = user_dig_vol_flag;
                if(user_dig_vol_step == 2)
                {
                    echo_led_on(TCFG_PWMLED_PIN0);
                    gpio_led_high_block(TCFG_PWMLED_PIN1);
    ...
    case SYS_DEVICE_EVENT:
        ...
        case DEVICE_EVENT_FROM_ADAPTER:
            ...
            case ADAPTER_EVENT_CONNECT :
                ...
                if(dev_num == 1)
                {
                    user_dig_vol_step = USER_DIGITAL_VOL_MAX - 2;
                    user_dig_vol_flag = user_dig_vol_step;
                }
                if(dev_num == 2)
                {
