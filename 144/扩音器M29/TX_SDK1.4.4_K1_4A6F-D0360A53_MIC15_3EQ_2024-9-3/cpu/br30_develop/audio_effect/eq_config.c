...
#define CALL_SECTION  10//下行段数,小于等于SONG_SECTION
#define UL_SECTION    10//上行段数,小于等于SONG_SECTION
/*
...
extern void user_led_flag(u8 num);

void eq_mode_delay_change(int mode)
{
    //int err = 0;
   // err = syscfg_write(CFG_USER_EQ_MODE, mode, 1);
    printf("mode is %d\n", (int)mode);
    if(mode == 1){
        eq_file_get_cfg(get_eq_cfg_hdl(), SDFILE_RES_ROOT_PATH"eq_cfg_hw.bin");
        //user_led_flag(1);
    }else if(mode == 2){

        eq_file_get_cfg(get_eq_cfg_hdl(), SDFILE_RES_ROOT_PATH"eq_cfg_hw1.bin");
        //user_led_flag(2);
    }else if(mode == 3){

        eq_file_get_cfg(get_eq_cfg_hdl(), SDFILE_RES_ROOT_PATH"eq_cfg_hw2.bin");
       // user_led_flag(3);
    }
}
#endif
