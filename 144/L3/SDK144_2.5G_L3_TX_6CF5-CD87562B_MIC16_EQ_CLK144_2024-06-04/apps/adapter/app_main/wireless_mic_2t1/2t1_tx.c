...
const struct __stream_HighSampleRateSingleMicSystem_parm denoise_parm_3 = {
    .AggressFactor = 1.0f,
    .minSuppress = 0.07f,/*!< 最小压制,建议范围[0.04 - 0.5], 越大降得越少 */
    .init_noise_lvl = 0.0f,
    .frame_len = WIRELESS_CODING_FRAME_LEN,
};
#endif
...
{
    adapter_adc_mic_data_read_r(in->data_len);

    user_audio_pcm_add_to_single(in->data,  in->data_len >> 1);
...
#endif
    {
        ...
        .value = {
            .encoder = {
                ...
                .data_handle = NULL,//encoder_pro_handler,//如果要关注处理前的数据可以在这里注册回调
                //其他关于encoder的参数配置
            },
        },
    },
...
#define USER_DIGITAL_VOL_MAX        10
u8 user_dig_vol_step = 6;
const u16 user_dig_vol_table[USER_DIGITAL_VOL_MAX + 1] = {
    0    , //0
    1347, //1
    3930,//2751, //2
    3930, //3
...
static int adapter_key_event_handler(struct sys_event *event)
{
...
#if 0//!ALWAYS_RUN_STREAM	//未连接也可以设置降噪档位
    ...
    case KEY_WIRELESS_MIC_DENOISE_SET:
        ...
        if(exit_mic_flag)
        {
            ...
            if (wireless_mic_media) {
                ...
                case 1:
                    //深度降噪
                    wl_mic_tone_play(TONE_DENOISE_ON, 1, NULL, NULL);
        ...
        if(exit_mic_flag)
        {
            ...
            if (wireless_mic_media)
            {
                if(echo_cmd == WIRELESS_MIC_ECHO_OFF)
                    {
                        wl_mic_tone_play(TONE_ECHO_OFF, 1, NULL, NULL);
                    ...
                    if(echo_cmd == WIRELESS_MIC_ECHO_ON)
                    {
                        wl_mic_tone_play(TONE_ECHO_ON, 1, NULL, NULL);
                        adapter_decoder_ioctrl(wireless_mic_media->downdecode, ADAPTER_DEC_IOCTRL_CMD_ECHO_EN, 1, (int *)(echo_cmd));
    ...
    case  KEY_EXIT_MIC:
        ...
        //if(connect_state_flag)
        {
            ...
            {
                printf("!!!!!!!!!!!!!!!!!1111111111\n");
                wl_mic_tone_play(TONE_RECODER_OFF, 1, NULL, NULL);
                exit_mic_flag = 0;
            ...
            {
                printf("!!!!!!!!!!!!!!!!!222222222222\n");
                wl_mic_tone_play(TONE_RECODER_ON, 1, NULL, NULL);
                exit_mic_flag = 1;
                //exit_led_flash_del();
    ...
    case KEY_OPEN_BLUETOOTH:
        bluetooth_ctl_flag = !bluetooth_ctl_flag;
        if(bluetooth_ctl_flag)
        {
            gpio_set_direction(IO_PORTC_04, 0);
            gpio_set_output_value(IO_PORTC_04, 1);
            wl_mic_tone_play(TONE_BT_ON, 0, NULL, NULL);
            bluetooth_dete_add();
        }
        else{
            bluetooth_dete_del();
            wl_mic_tone_play(TONE_BT_OFF, 0, NULL, NULL);
            gpio_set_direction(IO_PORTC_04, 0);
            gpio_set_output_value(IO_PORTC_04, 0);
        }
        break;
        case KEY_MIC_VOL_SET_UP:
        //if(connect_state_flag)
        {
            if(exit_mic_flag)
            {
                //syscfg_read(CFG_USER_MIC_VOL,&user_dig_vol_step,user_dig_vol_step);
                user_dig_vol_step += 2;
                if(user_dig_vol_step >= 10)
                {
                    wl_mic_tone_play(TONE_MAX_VOL, 0, NULL, NULL);

                    user_dig_vol_step = 10;
                }
                else{
                    wl_mic_tone_play(TONE_VOL_UP, 0, NULL, NULL);
                }
            }
        }

    break;
    case KEY_MIC_VOL_SET_DOWN:
        //if(connect_state_flag)
        {
            if(exit_mic_flag)
            {
                ...
                if(user_dig_vol_step <= 2)
                {
                    wl_mic_tone_play(TONE_MINE_VOL, 0, NULL, NULL);
                    user_dig_vol_step = 2;
                }
                else{
                    wl_mic_tone_play(TONE_VOL_DOWN, 0, NULL, NULL);
                }
    ...
    case SYS_DEVICE_EVENT:
        ...
        case DEVICE_EVENT_FROM_ADAPTER:
            ...
            case ADAPTER_EVENT_CONNECT :
                ...
                wl_mic_tone_play(TONE_MIC_CONN, 1, NULL, NULL);
                led_flash_del();
                echo_led_on(TCFG_PWMLED_PIN0);
            ...
            case ADAPTER_EVENT_DISCONN :
                ...
                wl_mic_tone_play(TONE_MIC_DISCONN, 1, NULL, NULL);
                led_flash_add();
