...
static u8 bluetooth_flag = 0;
bool bt_conn_success = FALSE;
void bluetooth_dete()
{
    ...
    {
        if(gpio_read(IO_PORTC_05))
        {
            wl_mic_tone_play(TONE_BT_CONN, 1, NULL, NULL);
            //sys_auto_shut_down_disable();
            bluetooth_flag = 0;
            bt_conn_success = TRUE;
        }
    }
    else if(bluetooth_flag == 0)
    {
        if(!gpio_read(IO_PORTC_05))
        {
            wl_mic_tone_play(TONE_BT_DISCONN, 1, NULL, NULL);
            //sys_auto_shut_down_enable();
            bluetooth_flag = 1;
            bt_conn_success = FALSE;
        }
    }
}

static u16 bluetooth_dete_time = 0;
void bluetooth_dete_add()
{
    if(!bluetooth_dete_time)
        bluetooth_dete_time = sys_timer_add(NULL,bluetooth_dete,1000);
}

void bluetooth_dete_del()
{
    if(bluetooth_dete_time)
    {
        sys_timer_del(bluetooth_dete_time);
        bluetooth_dete_time = 0;
    }
...
void ui_manage_scan(void *priv)
{
    ...
    if (sys_ui_var.other_status != STATUS_POWEROFF && sys_ui_var.power_status != STATUS_NORMAL_POWER) {  //关机的状态优先级要高于电源状态
        ...
        case STATUS_LOWPOWER:
            ...
            wl_mic_tone_play(TONE_LOW_POWER, 1, NULL, NULL);
    ...
    case STATUS_POWEROFF:
        ...
        wl_mic_tone_play(TONE_POWER_OFF, 1, NULL, NULL);
        int i = 0;
        for(i = 0;i < 2;i++)
        {
            pwm_led_mode_set(PWM_LED_ALL_OFF);
...
#if WIRELESS_MIC_CONNECT_SET
        mic_connect_set_wireless();
#endif
...
//        tone_play_index(TONE_BT_CONN, 1);
