...
struct user_mono_pcm_r_cbuf{

    cbuffer_t cbuf;
    s16 user_mono_data_r_tmp[500];
    u8  pcm_buf[2048];//[ADAPTER_PCM_BUF_SIZE];

};
struct user_mono_pcm_r_cbuf user_mono_pcm_r_cbuf ;
...
static inline void user_audio_pcm_dual_to_mono_lr(s16 *mono_pcm_l, s16 *mono_pcm_r, s16 *mono_pcm, int points)
{
    s16 *mono = mono_pcm;
    int i = 0;
    u8 j = 0;
...
#define  USER_MIC_DATA_PUT_NUM  10
u8 user_data_cnt = USER_MIC_DATA_PUT_NUM;
s16 user_mono_data_l[500] = {0};
s16 user_mono_data_r[500] = {0};

extern bool bt_conn_success;
...
static void adapter_adc_mic_output(void *priv, s16 *data, int len)
{
    ...
    #if 1
    extern int user_audio_digital_vol_run(void *data, u32 len, u32 l_vol,u32 r_vol);
    extern u8 user_dig_vol_step;
    const u16 user_dig_vol_table[11] = {
    0    , //0
    1347, //1
    3930,//2751, //2
    3930, //3
    ...
    15200,//11466,//8
    15200,//9
    20000,//16384 //10改小 15000？
    };
    //user_dig_vol_step = syscfg_read(CFG_USER_MIC_VOL,&user_dig_vol_step,user_dig_vol_step);
    user_audio_digital_vol_run(data,olen, user_dig_vol_table[user_dig_vol_step], user_dig_vol_table[user_dig_vol_step]);
    #endif
    //memset(data, 0, olen);
    //putchar('A');
#if 0
    ...
    //get_sine_data(&tx_s_cnt, data, olen / 2, 1);
#endif
    user_audio_pcm_dual_to_mono_lr(user_mono_data_l, user_mono_data_r, data, olen >> 1);
    if (adapter_adc_mic->data_callback) {
        ...
        int wlen = cbuf_write(&adapter_adc_mic->cbuf, user_mono_data_l, olen >> 1);
        if(bt_conn_success)
        {
            int wlen_r = cbuf_write(&user_mono_pcm_r_cbuf.cbuf, user_mono_data_r, olen >> 1);
        }
        //int wlen_r = cbuf_write(&user_mono_pcm_r_cbuf.cbuf, user_mono_data_r, olen >> 1); ///将R_ch声道声音存起来

       //if (wlen != olen >> 1 || wlen_r != olen >> 1)
       {
        // if (wlen != olen) {
            //putchar('M');
        }
...
void adapter_adc_mic_data_read_r(int data_len)
{
    if (adapter_adc_mic) {
        int rlen;
        int cbuf_len = cbuf_get_data_size(&user_mono_pcm_r_cbuf.cbuf);
        printf("cbuf_len %d\n", cbuf_len);
        if (cbuf_len >= data_len) {
            rlen = cbuf_read(&user_mono_pcm_r_cbuf.cbuf, user_mono_pcm_r_cbuf.user_mono_data_r_tmp, data_len);
            if (rlen != data_len) {
                printf("cbuf read err, rlen %d\n", rlen);
            }
             putchar('r');
        } else {
            memset(user_mono_pcm_r_cbuf.user_mono_data_r_tmp, 0, data_len);
            putchar('R');
        }
...
void user_audio_pcm_add_to_single(s16 *in_pcm,  int points)
{
    s16 *in_pcm_r = user_mono_pcm_r_cbuf.user_mono_data_r_tmp;
    while (points--) {
#if 0
        *out_pcm++ = data_sat_s16(in_pcm[0] + in_pcm[1]);
#else
        s32 tmp32 = (in_pcm[0] + in_pcm_r[0]);
        if (tmp32 < -32768) {
            tmp32 = -32768;
        } else if (tmp32 > 32767) {
            tmp32 = 32767;
...
int adapter_adc_mic_open(u16 sample_rate, u8 gain, u16 irq_points)
{
...
#endif
    {
        ...
        audio_mic_0dB_en(1);//linein默认关掉前级6dB
        audio_adc_mic1_set_gain(&hdl->mic_ch, gain);
        audio_mic1_0dB_en(1);//linein默认关掉前级6dB
    }
    ...
    cbuf_init(&user_mono_pcm_r_cbuf.cbuf, user_mono_pcm_r_cbuf.pcm_buf, irq_points * 2 * 3);
    printf(" user irq_points 123456 %d\n", irq_points * 2 * 3);
...
void adapter_adc_gain_set(u8 mic_gain, u8 linein_gain)
{
    if (adapter_adc_mic) {
        ...
        audio_adc_linein_set_gain(&adapter_adc_mic->mic_ch, linein_gain);
         ...
         //audio_adc_mic_set_gain(&adapter_adc_mic->mic_ch, linein_gain);
#elif CONFIG_CPU_BR30
