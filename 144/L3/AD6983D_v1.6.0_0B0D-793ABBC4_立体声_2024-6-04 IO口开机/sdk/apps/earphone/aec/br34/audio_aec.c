...
static void audio_aec_param_init(struct aec_s_attr *p)
{
    ...
    if (ret == sizeof(AEC_CONFIG)) {
        ...
        p->DNS_GainFloor = cfg.ans_aggress;
        p->DNS_OverDrive = cfg.ans_suppress;
        p->DNS_Loudness = 1.0f;
        ...
        p->DNS_OverDrive = 1.0f;   /*控制降噪强度，范围：0~6.0，越大降噪越强，正常降噪时为1.0，建议调节范围:0.3~3*/
...
int audio_aec_init(u16 sample_rate)
{
    ...
    extern u8 kws_is_running(void);
    if (kws_is_running()) {
        aec_clock = CONFIG_BT_CALL_KWS_HZ;
    }
#endif /*TCFG_KWS_VOICE_RECOGNITION_ENABLE*/
