...
static u8 music_flag = 0;
void music_detect()
{

    //f(get_bt_connect_status() == BT_STATUS_PLAYING_MUSIC)
    //{
        if(music_flag)
        {
...
void music_control_add()
{
    if(!music_time)
    {
        music_time = sys_timer_add(NULL,music_detect,200);
    }
}

void music_control_del()
{
    if(music_time)
    {
        sys_timeout_del(music_time);
        music_time = 0;
    }
}
