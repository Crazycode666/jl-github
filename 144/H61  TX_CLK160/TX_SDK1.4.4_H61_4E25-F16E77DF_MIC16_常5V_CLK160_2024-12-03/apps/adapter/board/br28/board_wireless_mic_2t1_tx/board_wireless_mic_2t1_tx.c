...
#define LDO_IN_INDEX 2 //有两处地方用到，用宏好控制
const struct wakeup_param wk_param = {
    .port[1] = &port0,
#if TCFG_CHARGE_ENABLE
    .aport[0] = &charge_port,
    .aport[1] = &vbat_port,
    .aport[LDO_IN_INDEX] = &ldoin_rise_port,
#endif

};

extern void power_wakeup_set_wakeup_io(u8 index, struct port_wakeup *port,u8 nanalog);
//设备ldeoIN的唤醒方向
void ldoin_wakeup_set_edge(u8 edge,u8 both_edge)
{
    ldoin_rise_port.edge = edge;
    ldoin_rise_port.both_edge = both_edge;
    power_wakeup_set_wakeup_io(LDO_IN_INDEX, &ldoin_rise_port, 0);
}
...
extern void set_ch_type(u8 ch);

static void wireless_tx_stereo_ch_set(void)
{
	u8 flag_tx_ch = 0;
	printf("wireless_tx_stereo_ch_set");
	gpio_set_direction(WIRELESS_TX_L_CH_IO,1);
	gpio_set_pull_down(WIRELESS_TX_L_CH_IO,0);
	gpio_set_pull_up(WIRELESS_TX_L_CH_IO,1);
	gpio_set_die(WIRELESS_TX_L_CH_IO,1);
	u8 cnt = 0;
	for (int i = 0;i<5;i++) {
		os_time_dly(1);
		if (gpio_read(WIRELESS_TX_L_CH_IO)) {
			printf("H");
			cnt++;
		} else {
			printf("L");
	...
	printf("flag_tx_ch = %d",flag_tx_ch);
	set_ch_type(flag_tx_ch);
}
#endif
void uartSendInit();
