...
{
    timer_pwm_init(JL_TIMER2,TCFG_PWMLED_PIN0,200,2000);
}

void red_led_init(void)
{
    timer_pwm_init(JL_TIMER4,TCFG_PWMLED_PIN1,200,800);
}
...
void led_flash_add()
{
    if(!led_time)
    {
        led_time = sys_timer_add(NULL,led_flash,100*5);
    }
}
...
void green_led_flash_add()
{
    if(!green_led_time)
    {
        green_led_time = sys_timer_add(NULL,green_led_flash,1000);
    }
}

void green_led_flash_del()
{
    if(green_led_time)
    {
        sys_timer_del(green_led_time);
        green_led_time = 0;
        gpio_disable_fun_output_port(TCFG_PWMLED_PIN0);
        gpio_led_high_block(TCFG_PWMLED_PIN0);
        gpio_disable_fun_output_port(TCFG_PWMLED_PIN1);
...
void red_led_flash_add()
{
    if(!red_led_time)
    {
        red_led_time = sys_timer_add(NULL,red_led_flash,1000);
    }
}
