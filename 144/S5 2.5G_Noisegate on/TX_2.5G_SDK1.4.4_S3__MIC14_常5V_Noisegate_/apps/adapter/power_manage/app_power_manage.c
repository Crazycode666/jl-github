...
void vbat_check_init(void)
{
    if (vbat_slow_timer == 0) {
        vbat_slow_timer = sys_timer_add(NULL, vbat_check_slow, 5 * 1000);
    } else {
        sys_timer_modify(vbat_slow_timer, 10 * 1000);
    }
...
void charge_vbat_led_states()
{
    ...
    if(power >= 20)
    {
        user_pwm_led_on(TCFG_PWMLED_PIN4);
        //rgb_led_off(TCFG_PWMLED_PIN3);
        //rgb_led_off(TCFG_PWMLED_PIN2);
        rgb_led_off(TCFG_PWMLED_PIN);
        return;
    }
    else
    {
        ...
        gpio_disable_fun_output_port(TCFG_PWMLED_PIN4);
        rgb_led_off(TCFG_PWMLED_PIN4);
        rgb_led_on(TCFG_PWMLED_PIN);
        //rgb_led_off(TCFG_PWMLED_PIN3);
